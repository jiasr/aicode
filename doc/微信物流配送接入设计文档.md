...
> **更新日期**：2026-09-03
> **修改说明**：第 4.3 节 `ExpressAccount` 表主键由 `INT AUTO_INCREMENT` 改为 UUID（`VARCHAR(32)`），以符合《开发规范汇总》一.1「新建表主键统一 UUID（uuid4().hex）」。（来源：CodeBuddy）
...
> **范围与定位（来源：用户需求，2026-09-03）**：本文档仅覆盖「快递」（`delivery_type=0`）这一配送方式的物流执行（电子面单 + 轨迹查询）。商城整体支持四种配送方式——**快递 / 外卖 / 自提 / 无物流**，由《配送方式与运费管理设计文档》统一定义，下单时用户任选其一；**本期先集成快递**（见第4章 Phase A）。外卖（同城即配，达达/美团等）、自提、无物流各有独立流程，不在本文档范围：外卖走即时配送平台、自提走门店核销、无物流无轨迹查询。
...
CREATE TABLE `t_mall_delivery_account` (
  ...
  `password` varchar(255) DEFAULT NULL COMMENT '密码(加密存储，建议 AES 后 Base64)',
...
> 本期落地 **Phase A（快递）**：微信物流助手 Handler + 电子面单 + 轨迹查询，对接《配送方式与运费管理设计文档》Phase 1。（来源：用户需求，2026-09-03）
